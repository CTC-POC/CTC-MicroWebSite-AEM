...
%><div id="<%=xssAttrWrap(xssAPI, cs.getId())%>">
    <div class="comment-signed-in-text" id="<%=xssAttrWrap(xssAPI, cs.getId())%>-signed-in-text"><%=xssFilterWrap(xssAPI, signedInText)%><span class="comment-signed-in-user" id="<%=xssAttrWrap(xssAPI, cs.getId())%>-signed-in-user"><%= StringEscapeUtils.escapeHtml4(formattedName) %></span></div>
...
$CQ(function(){
  ...
  <%if( isRTEenabled) {%>
  CQ.soco.commons.activateRTE($CQ("#<%=xssJSWrap(xssAPI, cs.getId())%>").find("form").first());
  <%}%>
    //CQ.soco.comments.attachToComposer($CQ("#<%=xssAPI.encodeForJSString(cs.getId())%>").find("form").first(), $CQ("#<%=xssAPI.encodeForJSString(cs.getId())%> ul.comments"), "comment");
    $CQ("#<%=xssJSWrap(xssAPI, cs.getId())%>").bind(CQ.soco.comments.events.ADDED, function(event){
            CQ_Analytics.record({event: 'postComment',
                    values: {commenterName: '<%=xssJSWrap(xssAPI, loggedInUserID)%>',
                            componentPath: '<%=resource.getResourceType()%>'
                            }
            });
            commentCount += 1;
  });
    $CQ("#<%=xssJSWrap(xssAPI, cs.getId())%>").bind(CQ.soco.comments.events.DELETE, function(event) {
    ...
    CQ.soco.comments.bindOnAdded($CQ("#<%=xssJSWrap(xssAPI, cs.getId())%>"));
});
</script>
