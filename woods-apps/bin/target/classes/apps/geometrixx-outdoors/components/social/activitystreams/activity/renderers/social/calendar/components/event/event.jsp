...
%>
        ...
        <div class="entries_content">
            ...
            <div class="desc">
                <p class="time border">
                    <span><%=xssAPI.encodeForHTML(author)%> <%=i18n.get("replied to")%> </span>
                    <a target="_blank" href="<%=xssAPI.getValidHref(objectPath)%>" title="<%=xssAttrWrap(xssAPI, activity.getVerb())%><%=i18n.get(" on ")%><%=xssAPI.encodeForHTML(target)%>">
                        <%= xssAPI.encodeForHTML(target)%>
                    </a>
