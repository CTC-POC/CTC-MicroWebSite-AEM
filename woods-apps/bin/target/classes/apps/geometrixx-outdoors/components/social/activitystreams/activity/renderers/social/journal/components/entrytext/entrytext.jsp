...
<%
    ...
    if(activity != null) {
        ...
        if (longMessage(message)) {
            ...
            //filter for attribute, don't want html here            
        }
        ...
        if(activityObject != null) {
            ...
            if (!ResourceUtil.isNonExistingResource(postResource) && ResourceUtil.isA(postResource, Forum.RESOURCE_TYPE_POST)) {
                Post topicObj = postResource.adaptTo(Post.class);
                objectPath  = topicObj.getUrl();
                forumSubject = topicObj.getSubject();
            }
        }
        //format subject
        ...
        forumSubject = forumSubject.replaceAll("\\r", "");
        forumSubject = forumSubject.replaceAll("\\n", "<br>");
        try {
            forumSubject = xssService.protectFromXSS(forumSubject);
        } catch (XSSProtectionException xe) {
        ...
        <div class="entries_content">
            ...
            <div class="desc">
                <p class="time border">
                    <span><%=xssAPI.encodeForHTML(author)%></span>
                    <span><%=xssFilterWrap(xssAPI, DateFormat.getDateInstance(DateFormat.MEDIUM).format(activity.getPublished()))%></span>
                </p>
                <a class="topic" target="_blank" href="<%=xssAPI.getValidHref(objectPath)%>" title="<%=xssAttrWrap(xssAPI, activity.getVerb())%> on <%=xssAPI.encodeForHTML(target)%>">
                    <%=xssAPI.filterHTML(forumSubject)%>
                </a>
