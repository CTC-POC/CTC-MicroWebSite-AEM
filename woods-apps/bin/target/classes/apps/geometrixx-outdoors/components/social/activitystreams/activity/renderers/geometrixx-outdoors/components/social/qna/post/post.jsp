...
<%
    ...
    if(activity != null) {
        ...
        <div class="entries_content">
            ...
            <div class="desc">
                <p class="time border">
                    <span><%=xssAPI.encodeForHTML(author)%> <%=i18n.get("answered")%>
                        
                        <a target="_blank" href="<%=xssAPI.getValidHref(objectPath)%>">"<%=xssAPI.filterHTML(qnaSubject)%>"</a>
                    </span>
                    <span><%=DateFormat.getDateInstance(DateFormat.MEDIUM).format(activity.getPublished())%></span>
                </p>

                <p class="top-comment italic comment-summary"><%=xssAPI.filterHTML(message)%><p>
            </div>
        </div>
