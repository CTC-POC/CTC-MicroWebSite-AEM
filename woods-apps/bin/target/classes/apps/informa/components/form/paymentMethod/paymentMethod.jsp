...
%><%@ page import="java.util.LinkedHashMap,
    ...
    if (displayValues == null) {
        ...
        String jsonURL="/apps/informa/components/form/paymentMethod/values.json/jcr:content";
		...
		Map<String,String> PaymentMethodItem = new LinkedHashMap<String,String>();
		if (jsonData != null) {     

    	JSONObject PaymentMethodObject = new JSONObject(jsonData);

    	JSONArray PaymentMethodArray = PaymentMethodObject.getJSONArray("root"); 

       for(int i = 0; i < PaymentMethodArray.length(); i++) {          

          JSONObject jsonObject = PaymentMethodArray.getJSONObject(i); 
           String PaymentMethodText=jsonObject.getString("text"); 
           String PaymentMethodValue=jsonObject.getString("value"); 
           PaymentMethodItem.put(PaymentMethodText,PaymentMethodValue);  

       } 

} 

String PaymentMethod = PaymentMethodItem.toString();
String[] PaymentMethods = PaymentMethod.substring(1, PaymentMethod.length()-1).split(",");
currentNode.setProperty("options",PaymentMethods);
currentNode.getSession().save();

        displayValues = PaymentMethodItem;

    }

    %>
<input type="hidden" value="<%=required%>" id="paymentmethodreq">
<input type="hidden" value="<%=requiredMessage%>" id="paymentmethodreqMesg">
<div class="form-group">
      <label for="paymentMethod" class="col-md-3 col-sm-3 form-one-control-label forms-title-color-fo"><%=title%>
<%if(required==true){%>
            <span class="asterisk-sign-clr">*</span>
            <%}%>
    </label>
       <div class="col-md-9 col-lg-7 col-sm-9 col-xs-11 input-div">
      <select name="paymentMethod" class="form-control custom-form-control selectpicker custom-select-box paymentMethod" data-size="5" id="paymentMethod">
