...
<div class="row-fluid mutli-col-article-summary"><%
    ...
    for (String pathProperty : articlePathProperties) {
        ...
        if (referencedPage != null) {
            ...
            String extension = xssAPI.encodeForHTMLAttr(GeoHelper.getPageImageExtension(referencedPage));
%>
            ...
            <div class="article-summary-image">
                <a href="<%= pagePath + ".html" %>">
                    <div data-picture data-alt='<%= alt %>'>
                        <div data-src='<%= pagePath + ".image.127.127.medium." + extension %>' data-media="(min-width: 1px)"></div>
                        <div data-src='<%= pagePath + ".image.127.127.medium." + extension %>' data-media="(min-width: 320px)"></div> <%-- Mobile --%>
                        <div data-src='<%= pagePath + ".image.620.290.high." + extension %>' data-media="(min-width: 481px)"></div>   <%-- Tablet --%>
                        <div data-src='<%= pagePath + ".image.770.360.high." + extension %>' data-media="(min-width: 1025px)"></div>  <%-- Tablet --%>
                        <%-- Fallback content for non-JS browsers. Same img src as the initial, unqualified source element. --%>
                        <noscript>
                            <img src='<%= pagePath + ".image.127.127.low." + extension %>' alt='<%= alt %>'>
                        </noscript>
                    </div>
                </a>
            </div>
