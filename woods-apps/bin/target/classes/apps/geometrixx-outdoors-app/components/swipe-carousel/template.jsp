    ...
    <c:otherwise>
        ...
        %>
                <div class="swipe-carousel-slide">
                    ...
                    <a x-cq-linkchecker="skip" class="carouselImageOverlay" ng-click="go('<%= request.getContextPath() %><%= xssAPI.encodeForHTMLAttr(carouselPage.getPath()) %>', '<%= xssAPI.encodeForJSString(title) %>')" title="<%= xssAPI.encodeForHTMLAttr(title) %>">
