...
%><%
    ...
    if (pageIterator.hasNext()) {
        ...
        if (referencedPage != null) {
            final String pagePath = request.getContextPath() + referencedPage.getPath();
            final String alt = referencedPage.getProperties().get("jcr:description", "");
            final String extension = GeoHelper.getPageImageExtension(referencedPage);
%>
        ...
        <div class="article-summary-image">
            <a href="<%= xssAPI.getValidHref(pagePath + ".html") %>">
                <div data-picture data-alt='<%= xssAPI.encodeForHTMLAttr(alt) %>'>
                    <div data-src='<%= xssAPI.encodeForHTMLAttr(pagePath + ".image.320.150.medium." + extension) %>' data-media="(min-width: 1px)"></div>
                    <div data-src='<%= xssAPI.encodeForHTMLAttr(pagePath + ".image.480.225.medium." + extension) %>' data-media="(min-width: 320px)"></div> <%-- Mobile --%>
                    <div data-src='<%= xssAPI.encodeForHTMLAttr(pagePath + ".image.620.290.high." + extension) %>' data-media="(min-width: 481px)"></div>   <%-- Tablet --%>
                    <div data-src='<%= xssAPI.encodeForHTMLAttr(pagePath + ".image.770.360.high." + extension) %>' data-media="(min-width: 1025px)"></div>  <%-- Tablet --%>
                    <%-- Fallback content for non-JS browsers. Same img src as the initial, unqualified source element. --%>
                    <noscript>
                        <img src='<%= xssAPI.encodeForHTMLAttr(pagePath + ".image.320.150.low." + extension) %>' alt='<%= xssAPI.encodeForHTMLAttr(alt) %>'>
                    </noscript>
                </div>
            </a>
        </div>
            ...
            <div class="article-summary-image">
                <a href="<%= xssAPI.getValidHref(pagePath + ".html") %>">
                    <div data-picture data-alt='<%= xssAPI.encodeForHTMLAttr(alt) %>'>
                        ...
                        <%-- Fallback content for non-JS browsers. Same img src as the initial, unqualified source element. --%>
                        <noscript>
                            <img src='<%= pagePath + ".image.127.127.medium." + extension %>' alt='<%= xssAPI.encodeForHTMLAttr(alt) %>'>
                        </noscript>
                    </div>
                </a>
            </div>
